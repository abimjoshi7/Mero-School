...
class BetterPlayerController {
  ...
  BetterPlayerController(
    ...
    if (betterPlayerDataSource != null) {
      print("==+>1. current url in datasource:225 ${betterPlayerDataSource.url}");
  ...
  Future setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    ...
    print("==+>1. current url in datasource: ${betterPlayerDataSource.url}");





    if(!betterPlayerDataSource.url.toString().contains("master.m3u8") && !betterPlayerDataSource.url.toString().contains(".mp4")){
      var myHttp  = Client();
      final response = await myHttp.get(Uri.parse(betterPlayerDataSource.url));
      Map<String, dynamic> responseJson = json.decode(response.body.toString());
      betterPlayerDataSource.url = "${responseJson['video_url']}/master.m3u8";

      if(betterPlayerConfiguration.byPassCache){
        var changed = betterPlayerDataSource.url.replaceFirst("https://video.mero.school", "https://originvideo.mero.school");
        // var changed = betterPlayerDataSource.url.replaceFirst("https://video.mero.school", "http://originvideoserver.mero.school");
        betterPlayerDataSource.url = changed;
      }
      ...
      print("withOrwithOUtBypass: ${betterPlayerDataSource.url}");

    }


    print("header in player: ${betterPlayerDataSource.headers}");
  ...
  Future _setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    ...
    var token = betterPlayerDataSource.headers?["Authorization"];
    ...
    if(betterPlayerDataSource.headers != null){
      _betterPlayerDataSource?.headers?.addAll(betterPlayerDataSource.headers!);
    }
  ...
  void jumpToVideo(int index){
    ...
    print("jumpPostion: "+ index.toString());
    _nextVideoTime = index;
  ...
  void playPreviousVideo() {
    ...
    _nextVideoTime = 0;
    _nextVideoTimeStreamController.add(-1);
    _postEvent(BetterPlayerEvent(BetterPlayerEventType.changedPlaylistItem));
    cancelNextVideoTimer();
  ...
  void setAlternatePlayer(String url){
    pause();
    postEvent(BetterPlayerEvent(BetterPlayerEventType.alternetPlayer,
        parameters: <String, dynamic>{
          "downloadUrl": url,
        }));
  ...
  void setDownloadAuto(String url){
    pause();
    postEvent(BetterPlayerEvent(BetterPlayerEventType.downloadClicked,
        parameters: <String, dynamic>{
          "downloadUrl": url,
        }));
  ...
  void setDownloadTrack(BetterPlayerAsmsTrack track, String url) {
    if (videoPlayerController == null) {
      throw StateError("The data source has not been initialized");
    }

    //pass the track and url to the viewplayer ui
    pause();
    postEvent(BetterPlayerEvent(BetterPlayerEventType.downloadClicked,
            parameters: <String, dynamic>{
              "downloadUrl": url,
