...
abstract class BetterPlayerControlsState<T extends StatefulWidget>
  ...
  void showSpeedChooserDialog(){

    _showSpeedChooserWidget();
  }

  void showQualityChooserDialog(){
    _showQualitiesSelectionWidget(false);
  }

  void showQualityChooserDownloadDialog(){
    // _showQualitiesSelectionWidget(true);
    _autoQualityDownload();
  }

  void showAlterPlayer(){
    _autoAlternetPlayer();
  }
  ...
  void _autoAlternetPlayer(){
    betterPlayerController!.setAlternatePlayer("${betterPlayerController?.betterPlayerDataSource?.url}");
  }


  void _autoQualityDownload(){
    betterPlayerController!.setDownloadAuto("${betterPlayerController?.betterPlayerDataSource?.url}");
  }
  ...
  void _showQualitiesSelectionWidget(bool isDownload) {
    // HLS / DASH
    ...
    for (var index = 0; index < asmsTracks.length; index++) {
      ...
      children.add(_buildTrackRow(asmsTracks[index], preferredName, isDownload));
    }
    ...
    if (children.isEmpty) {
      children.add(
        _buildTrackRow(BetterPlayerAsmsTrack.defaultTrack(),
            betterPlayerController!.translations.qualityAuto, isDownload),
      );
    }
  ...
  Widget _buildTrackRow(BetterPlayerAsmsTrack track, String? preferredName, bool isDownload) {
    ...
    return BetterPlayerMaterialClickableWidget(
      onTap: () {
        ...
        if(isDownload){
          betterPlayerController!.setDownloadTrack(track,"${betterPlayerController?.betterPlayerDataSource?.url}");
        }else{
  ...
  Widget _buildResolutionSelectionRowDownload(String name, String url) {
    final bool isSelected =
        url == betterPlayerController!.betterPlayerDataSource!.url;
    return BetterPlayerMaterialClickableWidget(
      onTap: () {
        ...
        print("bypass this: on resolution selected");
        print("bypass this: $url $name");
