...
class BetterPlayerPlaylistController {
  ...
  void _setup() {
    ...
    _nextVideoTimeStreamSubscription =
      ...
      if (time != null && time == 0) {
        _onVideoChange();
      }else if(time!=null && time == -1){
        _onVideoChangePrevious();
      }
    });
  }
  ...
  void _onVideoChange() {
    ...
    _betterPlayerController!.postEvent(BetterPlayerEvent(BetterPlayerEventType.updateLabel, parameters:<String, dynamic>{"nextIndex": nextDataSourceId}));

  }

  void _onVideoChangePrevious() {



    final int nextDataSourceId = _getPreviousDataSourceIndex();
  ...
  void setupDataSource(int index) {
    ...
    if (index <= _dataSourceLength) {
      ...
      print("==+>1. current url in datasource:145 ${_betterPlayerDataSourceList[index].url}");
